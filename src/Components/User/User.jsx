...
const User = ({user}) => {
    ...
    return (
        <div className='border-purple-400 border-2 bg-blue-200 p-5 text-center rounded-xl'>
            ...
            <Link to={`/client/${id}`}> <button className='bg-green-400 p-3 rounded-lg hover:bg-green-800 font-semibold'>Click Me</button></Link><br />
            <button className='bg-green-400 p-3 rounded-lg hover:bg-green-800 font-semibold mt-3'onClick={handleClikUserDetails}>Show User Details</button>
        </div>
    );
};
User.propTypes ={
